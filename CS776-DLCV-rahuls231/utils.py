...
def unpickle(datapaths):
    ...
    for file in os.listdir(datapaths):
        if (
                file == "data_batch_1" or file == "data_batch_2" or file == "data_batch_3" or file == "data_batch_4" or file == "data_batch_5"):
            print(file)
            ...
            if (test_data['data'].shape[0] == 0):
                ...
                test_data['data'] = np.vstack([test_data['data'], dict[b'data']])
...
def get_augmented_images(data, labels):
    ...
    for img in data:
        ...
        augmented_labels.append(labels[i])
        i += 1
    return np.array(augmented_img, dtype=np.uint8s), augmented_labels
